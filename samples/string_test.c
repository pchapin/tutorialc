...
LAST REVISION : 2001-11-13

(C) Copyright 2001 by Peter Chapin

This file contains a demo program that exercises some of the commonly
used functions declared in <string.h>. To compile this program use a
command such as

     $ cc -o string_test string_test.c -lcurses
 ...
 *
 * This function prepares curses for action. It sets appropriate screen
 ...
 *
 * This function cleans up curses and performs any other display related
 * clean up this application requires.
 *
 */
void reset_screen(void)
{
...
/*
 * void print_string(char *string, int length, int row, int col);
 *
 * This function prints the string pointed at by string onto the screen. It
 * starts at (row, col) and prints exactly length characters. It will print
 * past the null character if necessary. For characters that are not normal
 * printable characters, this function will print a special symbol directly
 * above the character and replace the character in the string with a '.'.
 *
 */
void print_string(char *string, int length, int row, int col)
{
 ...
 *
 * This function gets a line of text from the keyboard. It starts getting text
 * at the indicated coordinates. It echos the characters as it gets them. This
 * function is fairly rough, but it will be sufficient for this program.
 *
 */
void get_line(int row, int col, char *buffer)
{
...
int main(void)
{
  ...
  int  quit_flag = 0; // =1 when the user is sick of this program.
    ...
    switch (toupper(buffer[0])) {
      ...
      case 'I':
        ...
        if (result == NULL) {
          mvprintw(
            14, 0,
            "strchr(s, '%c') returns NULL. Strike any key.",
            buffer[0]
          );
          getch();
        }
          ...
          mvprintw(
            14, 0,
            "strchr(s, '%c') returns pointer as shown. Strike any key.",
            buffer[0]
          );
          getch();
          mvaddch(5, 19 + (result - s), ' ');
